...
При старте игра проверяет локальный endpoint `http://127.0.0.1:11434/api/tags`. Если он отвечает,
игра использует существующий Ollama и не завершает его при выходе. Иначе проверяется SHA-256
`tools/ollama/ollama.exe`, после чего CLI запускается скрыто с командой `serve`. Игра завершает
только запущенный ею процесс; ошибка проверки, запуска или подготовки модели выводится в статус UI.
...
Пока bundled CLI не добавлен в checkout, для разработки запустите Ollama отдельно и подготовьте модель:
