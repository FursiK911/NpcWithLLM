...
`LocalLlmRuntime` отправляет `POST` на `http://127.0.0.1:11434/api/chat` с `stream: false`, персоной,
памятью персонажа, ограниченной историей и текущим сообщением игрока. URL принимает только loopback;
